...
public function mentor_info($id="")

    {
        ...
        $data['page_title'] = "Katsina Wema Empowerment Program for MSMES";
        $email  = session()->get('email') ;
        if(($email == '')){ return redirect()->to(base_url('gfa/login')); }
        $data['email'] =  $email;
...
public function group_members_api()
{
...

    {
    	...
    	public function learning_wema($first_name="",$last_name="",$uid="",$program_type="",$course="",$platform=""){
                ...
                return redirect()->to('https://katsina-learning.remsana.com/portal/gfa/learning_path');
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            if (!empty($first_name) && !empty($last_name) && !empty($uid) && !empty($program_type) && !empty($course) && !empty($platform)) {
                ...
                return $this->response->setJSON([
                    ...
                    'url' =>'https://katsina-learning.remsana.com/portal/gfa/learning_wema/'.$first_name.'/'.$last_name.'/'.$uid.'/'.$program_type.'/'.$course.'/'.$platform.''
...

    {
        ...
        $email  = session()->get('email') ;
        if(($email == '')){ return redirect()->to(base_url('gfa/login')); }
        $title['page_title'] = "Katsina Wema Empowerment Program for MSMES";
        $data['email'] =  $email;
        $data['getPhoto']  =  $this->gfa_model->getPhotoUploaded($email);
    ...
    public function downloadmycertificate($ucode) {
        if(!empty($ucode)) {
            ...
            if($ucode == $profile_request[0]['ucode']) {
                $data['page_title'] = "Certificate of Completion - Katsina Wema Empowerment Program for MSMES";
                $data['certData'] = $profile_request;
            
                $html = view('certificate2', $data);
                $pdf = new Pdf();
                $certificate_name = 'Katsina Wema-certificate'.time().'.pdf';
                $pdf->generate($html, $certificate_name);
...
//                         }
    ...
    public function certificate_gen_course(){
        ...
        //$cert_type = array("fgn-alat-course","fgn-alat-soft");
        // Combine the parts to form the reference code
        $refCodeNysc = "WEMAKatsina{$currentYear}/{$currentMonth}/{$currentDay}/{$randomNumber}";
...

	{
        $email  = session()->get('email') ;
        if(($email == '')){ return redirect()->to(base_url('gfa/login')); }		
		$title['page_title'] = "Certificate Center - Katsina Wema Empowerment Program for MSMES";
       	$data['getStartUpDetails'] = $this->gfa_model->getStartUpDetails($email);
        $data['email'] =$email;
...

	{
        $email  = session()->get('email') ;
        if(($email == '')){ return redirect()->to(base_url('gfa/login')); }		
		$title['page_title'] = "Certificate not eligible - Katsina Wema Empowerment Program for MSMES";
...

	{
       ...
       if(($ref == '')){ return redirect()->to(base_url('gfa/login')); }		
		$data['page_title'] = "Certificate of Completion Soft Skills - Katsina Wema Empowerment Program for MSMES";
        ...
        $certificate_name = 'katsina-wema-certificate-soft'.time().'.pdf';
